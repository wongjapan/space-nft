...
contract Fight4HopeNFT is ERC721, ERC721Enumerable, ERC721Burnable, AccessControl, ERC1155Holder {
  ...
  function rentOut(
    ...
    rental[tokenId] = Rental({isActive: true, lord: msg.sender, renter: renter, expiresAt: expiresAt});
  ...
  function finishRenting(uint256 tokenId) external {
    ...
    require(msg.sender == _rental.renter || block.timestamp >= _rental.expiresAt, "RentableNFT: this token is rented");

    _rental.isActive = false;

    _transfer(_rental.renter, _rental.lord, tokenId);

    emit FinishedRent(tokenId, _rental.lord, _rental.renter, _rental.expiresAt);
  }
